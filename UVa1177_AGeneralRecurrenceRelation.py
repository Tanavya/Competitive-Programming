	...
	*/
	...
	while (T--) {
		...
		if (n == 0) cout << f0 << endl;
			...
			ld e1 = (a + sqrt(a*a+4*b))/2; 
			ld e2 = (a - sqrt(a*a+4*b))/2; 
			//ll ans = round(((power(e1,n)-power(e2,n)) * f1 + (power(e2,n)*e1-power(e1,n)*e2) * f0)/(e1-e2));
			ld ans = ((power(e1,n)-power(e2,n)) * f1 + (power(e2,n)*e1-power(e1,n)*e2) * f0)/(e1-e2);


			//assert(abs(round(ans)) <= 1e9);
			//assert(round(ans) == ans1);
			
			cout << setprecision(0) << fixed << ans1 << endl;
